...
func TestNew(t *testing.T) {
	tests := []struct {
		...
		{config: nil},
		{config: &config.Timeouts{Default: time.Second}},
		{config: &config.Timeouts{
			Default: time.Second,
			Overrides: []config.TimeoutsEntry{
				{Service: "foo", Method: "bar", Timeout: time.Minute},
			},
		}},
	}
...
func TestTimeoutOkay(t *testing.T) {
	m, err := New(&config.Timeouts{Default: time.Second}, nil, nil)
	assert.NoError(t, err)
...
func TestTimeoutWithinBoost(t *testing.T) {
	m, err := New(&config.Timeouts{Default: time.Nanosecond}, nil, nil)
	assert.NoError(t, err)
...
func TestTimeoutBlockForMoreThanBoost(t *testing.T) {
	...
	m, err := New(&config.Timeouts{Default: time.Nanosecond}, log, nil)
	assert.NoError(t, err)
...
func TestInfiniteTimeout(t *testing.T) {
	cfg := &config.Timeouts{Default: time.Duration(0)}
